...
The policies in `supabase/schema.sql` enforce:
...
```javascript
// In supabase-health-services.js
...
6. **Index properly** in Postgres for query performance

### Indexes (see `supabase/schema.sql`)
```json
// patients collection
...
2. **Transform to Supabase schema** adding:
...
| Queries failing | Confirm indexes in `supabase/schema.sql` are applied |
